...
function addFloatingCTA() {
    ...
    floatingButton.href = 'https://wa.me/2349059612256?text=Hi,%20I%20want%20to%20start%20learning';
...
function startCountdown() {
    const examDate = new Date('2026-03-05T00:00:00'); // JAMB registration date
    ...
    function updateCountdown() {
        ...
        if (diff > 0) {
            ...
            const monthsEl = document.querySelector('.countdown-item:nth-child(1) .countdown-number');
            const daysEl = document.querySelector('.countdown-item:nth-child(2) .countdown-number');
            const hoursEl = document.querySelector('.countdown-item:nth-child(3) .countdown-number');
            
            if (monthsEl) monthsEl.textContent = months;
            if (daysEl) daysEl.textContent = days.toLocaleString();
            if (hoursEl) hoursEl.textContent = hours.toLocaleString();
            
            // Add live seconds display
            const urgencyBadge = document.querySelector('.urgency-badge');
            if (urgencyBadge && !urgencyBadge.querySelector('.live-seconds')) {
                const secondsSpan = document.createElement('span');
                secondsSpan.className = 'live-seconds';
                secondsSpan.style.marginLeft = '10px';
                secondsSpan.style.fontWeight = 'normal';
                urgencyBadge.appendChild(secondsSpan);
            }
            
            const secondsDisplay = document.querySelector('.live-seconds');
            if (secondsDisplay) {
                secondsDisplay.textContent = `⏱️ ${seconds}s`;
            }
        }
    }
